...
def mstPrimEager(g):
    def include(i):
        # Mark vertex i as included in MST
        included[i] = True
        # For each edge adjacent to vertex i
        for e in g.adj[i]:
            x = e.other(i)  # Get the other vertex
            # If vertex x is not yet included in MST
            if not included[x]:
                # If x is not in pq yet, insert it
                if not pq.contains(x):
                    pq.insert(x, e)
                # If x is already in pq and the new edge has smaller weight
                elif e < pq.keyOf(x):
                    pq.decreaseKey(x, e)
    ...
    included = [False] * g.V  # included[v] == True if v is in the MST
    weightSum = 0  # Sum of edge weights in the MST
    pq = IndexMinPQ(g.V)  # Indexed min priority queue
    ...
    while not pq.isEmpty() and len(edgesInMST) < g.V - 1:
        e, w = pq.delMin()  # Get minimum edge and the vertex it connects to
        edgesInMST.append(e)  # Add edge to MST
        weightSum += e.weight  # Add weight to sum
        include(w)  # Include the new vertex
...
if __name__ == "__main__":
    ...
    g500 = WUGraph.fromFile("wugraph500.txt") 
    print("Prim eager on g500")
    edges, weightSum = mstPrimEager(g500)        
    if weightSum == 83208749: print ("pass")
    else: 
        print ("fail")
        failCorrectness = True
    print()
    ...
    if failCorrectness: print("fail - since the algorithm is not correct")
        ...
        n, mult, v = 5, 2, 500
        gs = WUGraph.fromFile(f"wugraph{v}.txt")
        tKruskal = timeit.timeit(lambda: mstKruskal(gs), number=n)/n
        tPrimLazy = timeit.timeit(lambda: mstPrimLazy(gs), number=n)/n
        tPrimEager = timeit.timeit(lambda: mstPrimEager(gs), number=n)/n
        print(f"Average running time for g{v} with Kruskal / {mult} ({tKruskal / mult:.10f}), PrimLazy / {mult} ({tPrimLazy / mult:.10f}), and PrimEager({tPrimEager:.10f})")
        if tPrimEager * mult < tKruskal and tPrimEager * mult < tPrimLazy: print ("pass")
        else: print ("fail")
    print()
