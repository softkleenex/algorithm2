...
def grahamScan(points):
    ...
    px, py  = min(points, key = lambda x : (x[1], -x[0])) #y는 작고, x는 커야한다!!!
    ...
    for i in range(len(points)):
        ...
        temp_angle = math.atan2(points[i][1] - py , points[i][0] - px)
    
        
        if temp_angle in points_angle:#동일 각도가 딕셔너리에 있다면 #points_angle[temp_angle]: #이렇게 하면 키 에러~~
            
            if (tx - px) ** 2 + (ty - py) ** 2 >=  (points_angle[temp_angle][0] - px) ** 2 + (points_angle[temp_angle][1] - py) ** 2: #startpoint에서 먼 거 우선
                points_angle[temp_angle] = [tx, ty] #px, py 였음,,
            else:
                pass    
        else:
            points_angle[temp_angle] =  [tx, ty] #px, py였음,,
        
       
    
    #points_angle에는 start_point기준으로, start_point를 제외하고 start_point기준의 각도들이 저장되어있다, 각도가 가장 큰것부터 빼내면 되니까, 각도 기준으로 정렬-셀정렬-이후에 pop하자!
    #좌표는 유일하지만, 각도는 유일하지 않다. 각도 : 좌표x, 좌표 y  딕셔너리를 만들자
    
    sorted_angle_keys = list(points_angle.keys())
    sorted_angle, temp = shellSort(sorted_angle_keys)
    ...
    now_points = [(px, py)]
    for i in range(len(sorted_angle)):
        now_points.append(tuple(points_angle[sorted_angle[i]]))
        
        # 점을 추가한 직후 볼록성 검사
        while len(now_points) >= 3 and not isconvex(now_points[-3], now_points[-2], now_points[-1]):
            now_points.pop(-2)
...
def isconvex(i, j, k):#점세개 a > b / b > c에 대해서, 점 b로 인해서 생기는 벡터가 볼록하다면 True > pass
   
    #교안에서는,, i > j > k 라면

    
    cross_product = (j[0] - i[0]) * (k[1] - i[1]) - (j[1] - i[1])*(k[0] -i[0])
    
    if cross_product > 0:#
        return True
    else:
        return False # ==0, 직선이거나,  < 0 이면 false, b를 제거해야한다
...
if __name__ == "__main__":
    ...
    input = [(4, 2), (3, -1), (2, -2), (1, 0), (0, 2), (0, -2), (-1, 1), (-2, -1), (-2, -3), (-3, 3), (-4, 0), (-4, -1), (-4, -2), (-4, -3), (-4, -4)]
    ...
    if not correct: print("Fail (since the algorithm is not correct)")
        ...
        print(f"Average running time of grahamScan() and simulateNSquare() * 0.2 with {inputLength} points: {tSubmittedCode:.10f} and {tCodeToCompare * 0.2:.10f}")                
        if tSubmittedCode < tCodeToCompare * 0.2: print("Pass")
        else:
            print("Fail")
        print() 